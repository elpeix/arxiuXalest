 ...
 * @dependences jQuery > 2.2.0
 *
 */
(function(){
    'use strict';
    
    // Variable principal assignada a window
    var MF = window.MF = {};
    ...
    MF.settings = {
        items_name: window.settings.items_name
    }
    ...
    var Model = MF.Model = function(options) {
        ...
        options = $.extend(false, self.constructor.prototype, options || {});
        ...
        function sync(_options){
            ...
            dataObj[type](JSON.stringify(self.content), async, function(rData){
                if (rData.status > 299) {
                    //Cagada TOTAL!
                    if (callback && typeof callback === 'function')
                        callback(rData);
    ...
    var Collection = MF.Collection = function(options) {
        ...
        var defaults = {
            ...
            model : null,
            isolateModel : false,
            items_name : MF.settings.items_name,
            transformURLs : null
        };
        ...
        function remove(identifier, callback){
            ...
            item.remove(function(rData){
                ...
                self[items_name].splice(index, 1);
        ...
        function isDuplicate(key, value, callback){
            var ret = false;
            for (var i = 0; i < self[items_name].length; i++){
                if (value == self[items_name][i].content[key]){
                    ret = parseInt(self[items_name][i].id);
                    break;
                }
            }
    ...
    /**
     * View Method - jQuery Plugin creator
     */
    var Views = MF.Views = function(name, o) {
        ;(function($) {
            ...
            $.fn[name] = function(options) {
                ...
                this.each(function(i, _element) {
                    ...
                    var obj = new o.ClassView(element, options);
    ...
    var extend = MF.extend = function(protoProps, staticProps) {
        var parent = this;
        var child;
        ...
        if (protoProps)
            $.extend(child.prototype, protoProps, staticProps);
